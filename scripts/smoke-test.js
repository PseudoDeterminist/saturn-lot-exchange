...
async function main() {
  ...
  const bestBuyTick = top[0];
  const buyLots = top[1];
  const bestSellTick = top[3];
  const sellLots = top[4];

  console.log('Top of book:', {
    bestBuyTick: bestBuyTick.toString(),
    buyLots: buyLots.toString(),
    bestSellTick: bestSellTick.toString(),
    sellLots: sellLots.toString()
  });

  if (sellLots.gt(0)) {
    console.log('Attempting buyFOK for 1 lot at sell top');
    const [levels, n] = await trade.getSellBook(1);
    if (n.toNumber() === 0) throw new Error('Unexpected: sell book empty');
    const lvl = levels[0];
    const price = ethers.BigNumber.from(lvl.price);
    const lots = ethers.BigNumber.from(1);
    const cost = price.mul(lots);
    const maxWetcIn = cost.mul(2);

    const bal = await wetc.balanceOf(deployerAddr);
    console.log('WETC balance:', bal.toString(), 'required:', cost.toString());
    if (bal.lt(cost)) throw new Error('Insufficient WETC balance for buyFOK test');

    const approveTx = await wetc.approve(tradeAddr, maxWetcIn);
    await approveTx.wait();
    console.log('Approved WETC to trade:', maxWetcIn.toString());

    const tx = await trade.buyFOK(bestSellTick, lots, maxWetcIn, history, { gasLimit: 8000000 });
  ...
  if (buyLots.gt(0)) {
    ...
    const [levels, n] = await trade.getBuyBook(1);
    if (n.toNumber() === 0) throw new Error('Unexpected: buy book empty');
    const lvl = levels[0];
    const price = ethers.BigNumber.from(lvl.price);
    const lots = ethers.BigNumber.from(1);
    const minWetcOut = price.mul(lots);

    const bal = await strn.balanceOf(deployerAddr);
    console.log('STRN10K balance:', bal.toString(), 'required lots:', lots.toString());
    if (bal.lt(lots)) throw new Error('Insufficient STRN10K balance for sellFOK test');

    const approveTx = await strn.approve(tradeAddr, lots);
    await approveTx.wait();
    console.log('Approved STRN10K to trade:', lots.toString());

    const tx = await trade.sellFOK(bestBuyTick, lots, minWetcOut, history, { gasLimit: 8000000 });
